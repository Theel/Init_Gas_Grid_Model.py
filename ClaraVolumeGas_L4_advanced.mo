within ;
model ClaraVolumeGas_L4_advanced "An array of  gas cells with dynamic momentum balance."
...
//__________________________________________________________________________//
  ...
  model Outline
    ...
    input ClaRa.Basics.Units.Velocity w[N_cv] "Velocity of flow in cells" annotation (Dialog(show));
...
// notice that in contrast to the simple L4 pipe for the dynamic momentuim balance this homoptopy relation is non  trivial and implies steady state start up.
 ...
 p_konv[1]=der(p[1])*geo.Delta_x[1]/fluid[1].w/2 - der(p[2])*geo.Delta_x[2]/fluid[2].w/2;
 p_konv[geo.N_cv+1]=der(p[geo.N_cv-1])*geo.Delta_x[geo.N_cv-1]/fluid[geo.N_cv-1].w/2 - der(p[geo.N_cv])*geo.Delta_x[geo.N_cv]/fluid[geo.N_cv].w/2;
 delta_p[1]=p[1] - p[2];
 delta_p[geo.N_cv+1]=p[geo.N_cv-1] - p[geo.N_cv];

for i in 2:geo.N_cv loop
    p_konv[i] = der(p[i-1])*geo.Delta_x[i-1]/fluid[i].w/2 - der(p[i])*geo.Delta_x[i]/fluid[i].w/2;
    delta_p[i] = p[i-1] - p[i];
end for;
...
end ClaraVolumeGas_L4_advanced;
